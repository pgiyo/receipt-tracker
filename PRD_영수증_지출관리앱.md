...
- [x] PDF 파일을 Upstage API에 직접 업로드 성공 (pdf2image/Poppler 불필요 확인)
- [x] Vercel 서버리스에서 `/tmp` 경로 사용 가능 여부를 공식 문서로 확인한다
- [x] LangChain + ChatUpstage(solar-pro) 조합의 OCR→JSON 파이프라인 예제 코드 확보 및 동작 확인

> **Phase 0 검증 결과 (2026-05-11)**
> - 올바른 모델명: `ocr` (PRD 명시 `document-digitization-vision` → 존재하지 않음)
> - OCR 파이프라인: `POST /v1/document-digitization` (model=ocr) → raw text → `ChatUpstage(solar-pro)` → JSON
> - PDF 직접 업로드 가능 → pdf2image/Poppler 의존성 제거
> - langchain-upstage 설치 버전: 0.7.7 (PRD 명시 0.1.0 → 최신 버전으로 교체 필요)
> - LLM 응답이 \`\`\`json 코드블록으로 감싸짐 → ocr_service.py에서 strip 처리 필요
> - Vercel /tmp: 512MB 쓰기 가능, cold start 시 초기화 → localStorage 병행 저장 필수
